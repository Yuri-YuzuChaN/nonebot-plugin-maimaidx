...
def song_list(songs: list[Song], page: int) -> str:
    ...
    _im.alpha_composite(ScoreBaseImage._aurora_bg.resize((1000, 174)))
    _im.alpha_composite(ScoreBaseImage._shines_bg.resize((1000, 442)))
    pattern = ScoreBaseImage._pattern_bg.resize((1000, 256))
    for h in range((height // 256) + 1):
        _im.alpha_composite(pattern, (0, (256 + 6) * h))
    _im.alpha_composite(
        ScoreBaseImage._rainbow_bg.resize((550, 288)), (225, height - 435)
    )
    _im.alpha_composite(
        ScoreBaseImage._rainbow_bottom_bg.resize((786, 164)),
        (107, height - 260),
    )
    ...
    im.alpha_composite(ScoreBaseImage._moon_bg.resize((120, 120)), (60, 20))
    im.alpha_composite(
    ...
    for num, song in enumerate(songs):
        ...
        im.alpha_composite(ScoreBaseImage._card_bg, (x, y))
        ...
        im.alpha_composite(
            ScoreBaseImage._sl_diff_utg
            if song.song_id > 100000
            else ScoreBaseImage._sl_diff_bg,
            (x + 100, y + 95),
        )
